function [total_response] = getResponseFunction(ca,lowest_mode_number,highest_mode_number,solvent_friction,time_step,eigvalues,eigvectors)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% input:
%   ca(PDB Structure)
...
mass_matrix = mass_matrix .* (1.66*10^(-27));
%-----------------------------------------------------
...
angular_frequency = sqrt(eigvalues.*4.184*10^26);
...
for m=1:M
	...
	if solvent_friction >= 2*angular_frequency(m)
		mode_response = 2/beta_1m*exp((-1)*solvent_friction*time_step/2)*sinh(beta_1m*time_step/2);
	else
		beta_1m=imag(beta_1m);
		mode_response = 2/beta_1m*exp((-1)*solvent_friction*time_step/2)*sin(beta_1m*time_step/2);
	end
